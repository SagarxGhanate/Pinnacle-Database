...
const Navbar = () => {
  ...
  const defaultAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'Admin')}&background=6D28D9&color=fff&size=128&bold=true&font-size=0.4`;
      ...
      {loggingOut && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-[#0e0e0e]">
          <div className="flex flex-col items-center animate-fadeIn">
            <div className="w-20 h-20 rounded-full bg-[#8B5CF6]/10 flex items-center justify-center mb-6 border-2 border-[#8B5CF6]/30 relative">
              <span className="material-symbols-outlined animate-spin text-[36px] text-[#8B5CF6]">progress_activity</span>
            </div>
            ...
            <div className="mt-6 w-48 h-1 bg-[#333333] rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#6D28D9] to-[#8B5CF6] rounded-full animate-[loadBar_1.3s_ease-in-out_forwards]"></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="relative flex justify-between items-center px-container-margin w-full max-w-[1440px] mx-auto h-full">
          <div className="flex items-center gap-8 h-full">
            <Link to="/dashboard" className="flex items-center gap-2.5">
              <img src="/images/logo/pinnacle-mark.png" alt="Pinnacle" className="h-[28px] w-auto drop-shadow-[0_0_12px_rgba(139,92,246,0.5)]" />
              <span className={`font-headline-sm text-[18px] font-bold tracking-tight ${isDark ? 'text-white' : 'text-[#1a1a2e]'}`}>
                Pinnacle <span className="text-[#8B5CF6] font-semibold">Forge</span>
              </span>
            </Link>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-6 xl:gap-8 h-full absolute left-1/2 -translate-x-1/2">
            <Link 
              to="/dashboard" 
              className={`h-full flex items-center font-label-md text-[14px] transition-colors duration-200 relative group ${isActive('/dashboard') ? 'text-[#8B5CF6] font-bold' : isDark ? 'text-on-surface-variant hover:text-white' : 'text-[#6B6B80] hover:text-[#1a1a2e]'}`}
            >
              Dashboard
              {isActive('/dashboard') && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#8B5CF6] rounded-t-full shadow-[0_-2px_10px_rgba(139,92,246,0.5)]"></div>}
            </Link>
            <Link 
              to="/members" 
              className={`h-full flex items-center font-label-md text-[14px] transition-colors duration-200 relative group ${isActive('/members') ? 'text-[#8B5CF6] font-bold' : isDark ? 'text-on-surface-variant hover:text-white' : 'text-[#6B6B80] hover:text-[#1a1a2e]'}`}
            >
              Members
              {isActive('/members') && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#8B5CF6] rounded-t-full shadow-[0_-2px_10px_rgba(139,92,246,0.5)]"></div>}
            </Link>
            <Link 
              to="/analytics" 
              className={`h-full flex items-center font-label-md text-[14px] transition-colors duration-200 relative group ${isActive('/analytics') ? 'text-[#8B5CF6] font-bold' : isDark ? 'text-on-surface-variant hover:text-white' : 'text-[#6B6B80] hover:text-[#1a1a2e]'}`}
            >
              Analytics
              {isActive('/analytics') && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#8B5CF6] rounded-t-full shadow-[0_-2px_10px_rgba(139,92,246,0.5)]"></div>}
            </Link>
            {(isOwner || isManager) && (
              <Link 
                to="/audit-log" 
                className={`h-full flex items-center font-label-md text-[14px] transition-colors duration-200 relative group ${isActive('/audit-log') ? 'text-[#8B5CF6] font-bold' : isDark ? 'text-on-surface-variant hover:text-white' : 'text-[#6B6B80] hover:text-[#1a1a2e]'}`}
              >
                Activity Log
                {isActive('/audit-log') && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#8B5CF6] rounded-t-full shadow-[0_-2px_10px_rgba(139,92,246,0.5)]"></div>}
              </Link>
            )}
            {isOwner && (
              <Link 
                to="/team" 
                className={`h-full flex items-center font-label-md text-[14px] transition-colors duration-200 relative group ${isActive('/team') ? 'text-[#8B5CF6] font-bold' : isDark ? 'text-on-surface-variant hover:text-white' : 'text-[#6B6B80] hover:text-[#1a1a2e]'}`}
              >
                Team
                {isActive('/team') && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#8B5CF6] rounded-t-full shadow-[0_-2px_10px_rgba(139,92,246,0.5)]"></div>}
              </Link>
            )}
          </nav>
          ...
          <div className="flex items-center gap-4">
              <button className={`p-2 rounded-xl transition-all active:scale-95 relative hidden sm:block ${isDark ? 'hover:bg-white/5 text-on-surface-variant hover:text-white' : 'hover:bg-black/5 text-[#6B6B80] hover:text-[#1a1a2e]'}`}>
                ...
                <span className="absolute top-2 right-2 w-2 h-2 bg-[#8B5CF6] rounded-full border border-[#0e0e0e]"></span>
              </button>
              ...
              <div className="relative" ref={profileRef}>
                ...
                >
                  <div className="relative">
                    <div className={`h-8 w-8 rounded-full overflow-hidden ring-2 transition-all ${isDark ? 'ring-[#8B5CF6]/30 group-hover:ring-[#8B5CF6]/60' : 'ring-[#8B5CF6]/20 group-hover:ring-[#8B5CF6]/50'}`}>
                    <img 
                      alt="Profile" 
              ...
              {logoutCountdown > 0 && (
                <div className="w-full mb-5">
                  <div className={`h-1 rounded-full overflow-hidden ${isDark ? 'bg-[#333333]' : 'bg-[#E8E8ED]'}`}>
                    <div 
                      className="h-full bg-gradient-to-r from-[#EF4444] to-[#8B5CF6] rounded-full transition-all duration-1000 ease-linear"
