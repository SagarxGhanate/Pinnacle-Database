...
const ROLE_CONFIG = {
  OWNER:   { label: 'Owner',   color: 'bg-[#8B5CF6]/10 text-[#8B5CF6] border-[#8B5CF6]/20' },
      ...
      <div className={`min-h-screen ${isDark ? 'bg-[#0e0e0e]' : 'bg-[#F5F5F7]'}`}>
        ...
        <div className="flex items-center justify-center h-[60vh]">
          <div className="text-center">
            ...
            <Link to="/dashboard" className="inline-block mt-4 text-[13px] text-[#8B5CF6] hover:underline">← Back to Dashboard</Link>
          </div>
        </div>
        <BottomNav />
      </div>
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-[#0e0e0e]' : 'bg-[#F5F5F7]'}`}>
      ...
      <main className="max-w-[800px] mx-auto px-4 md:px-8 py-8 pb-24">
        ...
        <div className="flex items-end justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Link to="/settings" className="text-[#6B6B80] hover:text-[#8B5CF6] transition-colors">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#8B5CF6] text-white font-headline font-bold text-[13px] rounded-lg hover:opacity-90 active:scale-[0.98] transition-all flex items-center gap-1.5 shadow-[0_4px_14px_rgba(249,115,22,0.3)]"
          >
          ...
          <div className={`rounded-2xl p-6 mb-6 transition-colors duration-300 ${
            ...
            {tempPassword && (
              <div className="mb-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                ...
                <div className="flex items-center gap-2">
                  <code className="px-3 py-1.5 bg-[#0e0e0e] rounded-lg text-[16px] font-mono font-bold text-[#8B5CF6] tracking-wider">
                    {tempPassword}
                  </code>
                  <button
            ...
            <form onSubmit={handleInvite} className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                className={`flex-1 h-[40px] rounded-xl px-3 outline-none text-[13px] transition-all focus:ring-2 focus:ring-[#8B5CF6]/30 ${
              ...
              <input
                ...
                className={`w-full sm:w-[140px] h-[40px] rounded-xl px-3 outline-none text-[13px] transition-all focus:ring-2 focus:ring-[#8B5CF6]/30 ${
                  isDark ? 'bg-[#262626] border border-white/[0.08] text-[#EEEEF0]' : 'bg-[#F0F0F5] border border-gray-200 text-[#1a1a2e]'
                }`}
              ...
              <button
                type="submit"
                disabled={inviting}
                className="h-[40px] px-5 bg-[#8B5CF6] text-white font-bold text-[13px] rounded-xl hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-70"
              >
        ...
        <div className={`rounded-2xl p-4 mb-3 flex items-center gap-4 ${
          isDark ? 'bg-[#1a1a1a] border border-[#8B5CF6]/20' : 'bg-white border border-[#8B5CF6]/20'
        }`}>
          <div className="w-10 h-10 rounded-full bg-[#8B5CF6]/10 flex items-center justify-center flex-shrink-0">
            <span className="material-symbols-outlined text-[20px] text-[#8B5CF6]">shield_person</span>
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {m.initialPassword && (
                      <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-[#8B5CF6]/10 border border-[#8B5CF6]/20">
                        <span className="text-[10px] text-[#8B5CF6] font-bold uppercase tracking-wider">Pass:</span>
                        <code className="text-[11px] text-[#8B5CF6] font-mono font-bold tracking-widest">{m.initialPassword}</code>
                        <button
                          onClick={() => { navigator.clipboard.writeText(m.initialPassword); toast.success('Password copied!'); }}
                          className="ml-1 p-0.5 rounded text-[#8B5CF6] hover:bg-[#8B5CF6]/20 transition-colors"
                          title="Copy Password"
                        >
